...
| `atom_ref` | `<source_file>#<row_or_id>` | Pointer back to the evidence file row (e.g. `i_calendar_meetings.md#row-14`, `f_slack_dms.md#row-3`) |
...
| `AI-training-meet` | Dedicated AI tools training session (from `i_calendar_meetings.md` tag `ai-training`) |
...
| date | atom_ref | type | mentee | topic | outcome | subcat_tags |
|------|----------|------|--------|-------|---------|-------------|
| 2026-03-12 | i_calendar_meetings.md#row-8 | AI-training-meet | team | Cursor composer workflow | 5 engineers adopted Cursor in next sprint | 1184,1187 |
...
1. Read `i_calendar_meetings.md` — extract rows with tag `ai-training`, `mentoring`, or `1:1` where topic involves AI tools
...
4. Read `j_shoutouts.md` — extract rows that mention AI tools, skills, or teaching
